...
void Bme280::begin() {
    ...
    uint8_t in[30];
    out[0] = 0x88;
    int err = Wire.transfer(0x76, out, 1, in, 28);
...
bool Bme280::measureTemperature() {
    ...
    if (err == 0)
    {
        uint32_t adc_T = ((uint32_t) in[0] << 12) | (in[1] << 4)
                | (in[2] >> 4);
...
bool Bme280::measureHumidity() {
    ...
    if (err == 0)
    {
        ...
        v_x1_u32r = (((((adc_H << 14) - (((int32_t) dig_H4) << 20)
                - (((int32_t) dig_H5) * v_x1_u32r)) + ((int32_t) 16384))
                >> 15)
...
bool Bme280::measurePreassure() {
    ...
    if (err == 0)
    {
        uint32_t adc_P = ((uint32_t) in[0] << 12) | (in[1] << 4)
                | (in[2] >> 4);
        ...
        uint32_t p_local = (((uint32_t) (((int32_t) 0x100000) - adc_P) - (var5 >> 12)))
                * 0xC35;
